...
use crate::order::fields::{ModificationId, OrderId, Quantity, Timestamp};
use crate::order::modification::{Modification, ModificationInstruction};

// ── Resting Order ─────────────────────────────────────────────────────────────

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct LimitRestingOrder {
    pub id: OrderId,
    pub timestamp: Timestamp,
    pub quantity: Quantity,
}

impl LimitRestingOrder {
...
mod tests {
    ...
    fn inspect_order_size() {
        println!(
            "LimitRestingOrder: {} bytes",
            std::mem::size_of::<LimitRestingOrder>()
        );
    }

    fn make_order() -> LimitRestingOrder {
        LimitRestingOrder::new(OrderId::new(1), Timestamp::new(1000), Quantity::new(100))
    }
